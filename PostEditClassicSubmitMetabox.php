...
class PostEditClassicSubmitMetabox
{
                    ...
                    <div class="misc-pub-section misc-pub-visibility" id="visibility">
                        ...
                        <span id="post-visibility-display">
                            ...
                            if ( 'private' === $post->post_status ) {
                                $post->post_password = '';
                                $visibility          = 'private';
                                $visibility_trans    = __('Private', 'publishpress-statuses');
                            } elseif ( ! empty( $post->post_password ) ) {
                                $visibility       = 'password';
                                $visibility_trans = __('Password protected', 'publishpress-statuses');
                            } elseif ( 'post' === $post->post_type && is_sticky( $post->ID ) ) {
                                $visibility       = 'public';
                                $visibility_trans = __('Public, Sticky', 'publishpress-statuses');
                            } else {
                                $visibility       = 'public';
                                $visibility_trans = __('Public', 'publishpress-statuses');
                            }
                        ...
                        <?php if ( $can_publish ) { ?>
                            <a href="#visibility" class="edit-visibility hide-if-no-js" role="button"><span aria-hidden="true"><?php _e( 'Edit', 'publishpress-statuses' ); ?></span> <span class="screen-reader-text">
                                <?php
                                /* translators: Hidden accessibility text. */
                                _e( 'Edit visibility', 'publishpress-statuses' );
                                ?>
                            </span></a>
                            ...
                            <div id="post-visibility-select" class="hide-if-js">
                                ...
                                <input type="radio" name="visibility" id="visibility-radio-public" value="public" <?php checked( $visibility, 'public' ); ?> /> <label for="visibility-radio-public" class="selectit"><?php _e( 'Public', 'publishpress-statuses' ); ?></label><br />

                                <?php if ( 'post' === $post->post_type && current_user_can( 'edit_others_posts' ) ) : ?>
                                    <span id="sticky-span"><input id="sticky" name="sticky" type="checkbox" value="sticky" <?php checked( is_sticky( $post->ID ) ); ?> /> <label for="sticky" class="selectit"><?php _e( 'Stick this post to the front page', 'publishpress-statuses' ); ?></label><br /></span>
                                <?php endif; ?>

                                <input type="radio" name="visibility" id="visibility-radio-password" value="password" <?php checked( $visibility, 'password' ); ?> /> <label for="visibility-radio-password" class="selectit"><?php _e( 'Password protected', 'publishpress-statuses' ); ?></label><br />
                                <span id="password-span"><label for="post_password"><?php _e( 'Password:', 'publishpress-statuses' ); ?></label> <input type="text" name="post_password" id="post_password" value="<?php echo esc_attr( $post->post_password ); ?>"  maxlength="255" /><br /></span>

                                <input type="radio" name="visibility" id="visibility-radio-private" value="private" <?php checked( $visibility, 'private' ); ?> /> <label for="visibility-radio-private" class="selectit"><?php _e( 'Private', 'publishpress-statuses' ); ?></label><br />

                                <p>
                                    <a href="#visibility" class="save-post-visibility hide-if-no-js button"><?php _e( 'OK', 'publishpress-statuses' ); ?></a>
                                    <a href="#visibility" class="cancel-post-visibility hide-if-no-js button-cancel"><?php _e( 'Cancel', 'publishpress-statuses' ); ?></a>
                                </p>
                            </div>
                        <?php } ?>
                    </div>
                    ...
                    if (!empty($args['args']['revisions_count']) && apply_filters('pp_statuses_display_revisions_ui', true, $post)) :
                        ...
                        <div class="misc-pub-section num-revisions">
                            ...
                            if ($revisions_to_keep > 0 && $revisions_to_keep <= $args['args']['revisions_count']) {
                                ...
                                printf(esc_html__('Revisions: %s', 'publishpress-statuses'), '<b>' . (int) number_format_i18n($args['args']['revisions_count']) . '+</b>');
                                echo '</span>';
                            } else {
                                printf(esc_html__('Revisions: %s', 'publishpress-statuses'), '<b>' . (int) number_format_i18n($args['args']['revisions_count']) . '</b>');
                            }
            ...
            <div id="major-publishing-actions">
                ...
                <div id="delete-action">
                    ...
                    if (current_user_can("delete_post", $post->ID)) {
                        if (!EMPTY_TRASH_DAYS)
                            $delete_text = __('Delete Permanently', 'publishpress-statuses');
                        else
                            $delete_text = __('Move to Trash', 'publishpress-statuses');
                        ?>
    ...
    public static function post_save_button($post, $args)
    {
        ...
        <?php elseif ($is_moderation && ('future' != $post_status_obj->name)) :
            if (apply_filters('presspermit_display_save_as_button', true, $post, $args)):
                $save_caption = isset($post_status_obj->labels->save_as) ? $post_status_obj->labels->save_as : __('Save', 'publishpress-statuses');
                ?>
            ...
            ?>
        <?php else : ?>
            <input type="submit" name="save" id="save-post" value="<?php echo esc_attr(__('Save', 'publishpress-statuses')); ?>"
    ...
    public static function post_preview_button($post, $args)
    {
        ...
        if ($post_status_obj->public) {
            ...
            $preview_button = __('Preview', 'publishpress-statuses');
            $preview_title = '';
        } else {
            ...
            $preview_button = apply_filters('presspermit_preview_post_label', __('Preview', 'publishpress-statuses'));
    ...
    public static function post_status_display($post, $args)
    {
        ...
        ?>
        <label for="post_status"><?php echo esc_html__('Status:', 'publishpress-statuses'); ?></label>
        <?php
        ...
        if (!empty($post_status_obj->private))
            echo esc_html__('Privately Published', 'publishpress-statuses');
        elseif (!empty($post_status_obj->public))
            echo esc_html__('Published', 'publishpress-statuses');
        ...
        if (!empty($post_status_obj->public) || !empty($post_status_obj->private) || $can_publish || $select_moderation) { 
            ...
            <?php } ?>class="edit-post-status hide-if-no-js" tabindex='4'><?php esc_html_e('Edit', 'publishpress-statuses'); ?></a>
            ...
            <div id="post-status-select" class="hide-if-js">
                ...
                <a href="#post_status" class="save-post-status hide-if-no-js button"><?php echo esc_html__('OK', 'publishpress-statuses'); ?></a>

                <div class="pp-status-cancel">
                <a href="#post_status" class="pp-cancel-post-status hide-if-no-js"><?php echo esc_html__('Cancel', 'publishpress-statuses'); ?></a>
                </div>
    ...
    public static function post_time_display($post, $args)
    {
        ...
        $datef = __('M j, Y @ G:i', 'publishpress-statuses');
        ...
        if (0 != $post->ID) {
            ...
            if ('future' == $post_status_obj->name) { // scheduled for publishing at a future date
                ...
                printf(__('Scheduled for: %s', 'publishpress-statuses'), '<b>' . esc_html($date) . '</b>');

            } elseif (in_array($post_status_obj->name, $published_stati)) { // already published
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                printf(__('Published on: %s', 'publishpress-statuses'),  '<b>' . esc_html($date) . '</b>');
                ...
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                _e('Publish <b>immediately</b>', 'publishpress-statuses');

            } elseif (time() < strtotime($post->post_date_gmt . ' +0000')) { // draft, 1 or more saves, future date specified
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                printf(esc_html__('Schedule for: %s', 'publishpress-statuses'), '<b>' . esc_html($date) . '</b>');
                ...
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                printf(__('Publish on: %s', 'publishpress-statuses'), '<b>' . esc_html($date) . '</b>');
            } else {
        ...
        ?></span>
        <a href="#edit_timestamp" class="edit-timestamp hide-if-no-js" tabindex='4'><?php esc_html_e('Edit', 'publishpress-statuses'); ?></a>
        ...
        if ((empty($post_status_obj->public) && empty($post_status_obj->private) && ('future' != $post_status_obj->name))) {
            ...
            if (!empty($status_obj->public) || !empty($status_obj->private)) :
                ...
                <?php
                    ...
                    <input name="publish" type="submit" class="button button-primary button-large" id="publish" tabindex="5" accesskey="p" value="<?php echo esc_attr(__('Update', 'publishpress-statuses')); ?>"/>
                <?php
                endif;
            else :
            ...
            <input name="original_publish" type="hidden" id="original_publish" value="<?php echo esc_attr(__('Update', 'publishpress-statuses')); ?>"/>
            <input name="save" type="submit" class="button button-primary button-large" id="publish" tabindex="5" accesskey="p" value="<?php echo esc_attr(__('Update', 'publishpress-statuses')); ?>"/>
            <?php
        }
    }
}