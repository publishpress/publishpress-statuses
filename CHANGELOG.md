...
- Changed : Statuses table row action Edit link in Name column #317
- Changed : Statuses, Settings links on Plugins screen row #305
- Changed : Visual indicator that Draft, Pending Review statuses can't be disabled #98
...
- Fixed: Compat - Advanced Custom Fields - Update attempts with missing required fields left Publish / Update button hidden
...
- Fixed : Import -  On sites with imported PublishPress Planner statuses, plugin de/re-activation modified the position and enable / disable of some statuses
- Fixed : Import -  Planner 3.x import - some inconsistencies in the how status positions were imported
- Fixed : Import -  Permissions Pro 3.x import - status post types, nesting, labels were not imported
- Fixed : Import -  On deactivation, encoded status properties used by Planner 3.x were not restored
- Fixed : Import -  On deactivation, Planner 3.x post types settings  (using "on" / "off" value storage) were not restored
- Fixed : Import -  Option to re-import Planner configuration, with or without Permissions Pro Status Control properties
- Fixed : Import -  Failsafe mechanism disables auto-import if last attempt did not complete normally
- Added : Automatic and Manual backup of colors, icons, labels and post types for all statuses
- Added : Restore status colors, icons, labels or post types from automatic or manual backup
- Added : Revert status  colors, icons, labels or post types to defaults
- Added : Revert status  colors, icons, labels or post types to Planner defaults
- Changed : Rearranged plugin settings UI and clarified some captions
...
- Fixed: Compat - Permissions Pro - Prevent Permissions from causing a fatal error on Theme Customizer access
...
- Added : Classic Editor - When defaulting to next status, checkbox under publish button allows bypassing sequence; default-select after future date selection
- Added : Classic Editor - Implement capability pp_bypass_status_sequence to regulate availability of sequence bypass checkbox
...
- Changed : Posts / Pages screen - Eliminate redundant Status column
...
- Changed : Posts / Pages screen - If Private checkbox in Quick Edit is clicked, set Status dropdown to Published
- Changed : Posts / Pages screen - If Status dropdown in Quick Edit is set to something other than Published, uncheck Private checkbox
...
- Changed : New plugin setting "De-clutter status dropdown by hiding statuses outside current branch"; no longer do this by default
